...
export class JavaUniversalParser {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!currentFn) {
        const match = trimmed.match(
          /\b(?:public|private|protected|static|final|abstract|synchronized|\s)*[a-zA-Z0-9_<>\[\],\s]+?\s+([a-zA-Z0-9_]+)\s*\(([^)]*)\)/
        );
        if (match) {
          const name = match[1];
          const firstWord = trimmed.split(/\s+/)[0];

          if (!reservedWords.has(name) && !reservedWords.has(firstWord) && !trimmed.endsWith(';')) {
            currentFn = {
              name,
              startLine: lineNum,
  ...
  private extractLoops(lines: { line: number; text: string }[]): UniversalLoopNode[] {
    ...
    for (const l of lines) {
      ...
      if (isFor || isWhile) {
        ...
        let explanation = 'Laço Java com passo linear O(n)';
        ...
        if (
          ...
          explanation = 'Laço Java com passo multiplicativo/divisivo -> O(log n)';
        }
        ...
        if (stack.length > 0 && stack[stack.length - 1].loopKind === 'while') {
          if (
            ...
            stack[stack.length - 1].explanation =
              'Laço while Java com alteração multiplicativa/divisiva -> O(log n)';
          }
        }
        if (l.text.includes('}')) {
          if (stack.length > 0) {
